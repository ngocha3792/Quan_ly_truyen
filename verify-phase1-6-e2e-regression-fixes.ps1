...
if ($LASTEXITCODE -ne 0) { throw "Admin E2E seed lần 1 failed" }
npm --prefix backend run db:seed:e2e:admin
if ($LASTEXITCODE -ne 0) { throw "Admin E2E seed lần 2 failed" }
...
Write-Host "Bây giờ chạy full suite: npm --prefix frontend run e2e"
