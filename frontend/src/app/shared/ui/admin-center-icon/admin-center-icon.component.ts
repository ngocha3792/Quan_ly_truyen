...
export type AdminCenterIconName =
  | 'activity' | 'ads' | 'bell' | 'book' | 'calendar' | 'category'
  | 'chart' | 'check' | 'chevron-down' | 'coin' | 'comment'
  | 'dashboard' | 'edit' | 'eye' | 'filter' | 'flag' | 'image'
  | 'lock' | 'logout' | 'menu' | 'more' | 'plus' | 'search'
  | 'settings' | 'shield' | 'star' | 'trash' | 'user' | 'users';
