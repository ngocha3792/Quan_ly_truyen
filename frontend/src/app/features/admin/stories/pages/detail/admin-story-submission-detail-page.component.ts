...
export class AdminStorySubmissionDetailPageComponent implements OnInit {
  ...
  approve(): void {
    if (!window.confirm('Duyệt submission này?')) return;
    this.run(this.api.approve(this.id), 'Đã duyệt truyện.');
  }
  openReject(): void {
    this.rejectReason = '';
  ...
  closeReject(): void {
    if (!this.mutating()) this.rejectOpen.set(false);
  }
  confirmReject(): void {
    const reason = this.rejectReason.trim();
    if (reason.length < 10 || reason.length > 1000) {
      this.error.set('Lý do phải từ 10 đến 1000 ký tự.');
      return;
    }
    this.run(this.api.reject(this.id, reason), 'Đã từ chối truyện.', true);
  }
  private run(request: Observable<unknown>, success: string, closeDialog = false): void {
    ...
    request
      ...
      .subscribe({
        next: () => {
          if (closeDialog) this.rejectOpen.set(false);
