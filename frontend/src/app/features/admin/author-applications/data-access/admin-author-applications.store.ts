...
export class AdminAuthorApplicationsStore {
  ...
  readonly listStatus = signal<AdminAuthorApplicationLoadStatus>('idle');

  readonly detailStatus = signal<AdminAuthorApplicationLoadStatus>('idle');

  readonly actionStatus = signal<AdminAuthorApplicationActionStatus>('idle');
  ...
  readonly totalPages = computed(() =>
    Math.max(
      ...
      Math.ceil(this.totalState() / this.pageSize),
    ),
  );

  readonly isReviewing = computed(() => this.actionStatus() !== 'idle');
  ...
  loadDetail(applicationId: string): void {
    const revision = this.lifecycle.revision();
  ...
  approve(): Observable<AdminAuthorApplicationRecord> {
    const application = this.detailState();
    ...
    this.actionStatus.set('approving');

    this.actionError.set('');

    this.actionMessage.set('');

    return this.api.approve(application.applicationId).pipe(
      takeUntil(this.lifecycle.changes$),
      ...
      tap((updated) => {
        ...
        this.detailState.set(updated);

        this.actionMessage.set('Hồ sơ đã được duyệt và tài khoản tác giả đã được kích hoạt.');
      }),
  ...
  reject(reason: string): Observable<AdminAuthorApplicationRecord> {
    ...
    this.actionStatus.set('rejecting');
    ...
    return this.api
      ...
      .pipe(
        ...
        tap((updated) => {
          ...
          this.actionMessage.set('Hồ sơ đã bị từ chối. Người dùng có thể chỉnh sửa và gửi lại.');
        }),
