...
export class StoryRankingStore {
    private readonly repository =
        inject(StoryRankingRepository);

    private readonly destroyRef =
        inject(DestroyRef);

    private readonly queryState =
        signal<StoryRankingQuery>(
            DEFAULT_STORY_RANKING_QUERY,
        );

    private readonly overviewState =
        signal<StoryRankingOverview>(
            EMPTY_STORY_RANKING_OVERVIEW,
        );
    ...
    readonly topStories = computed(
        () =>
            this.stories().slice(0, 3),
    );

    readonly remainingStories =
        computed(
            () =>
                this.stories().slice(3),
        );
