...
export class MyLibraryStore {
    private readonly repository =
        inject(MyLibraryRepository);

    private readonly viewState =
        signal<MyLibraryView | null>(null);

    readonly view = this.viewState.asReadonly();

    readonly query = signal('');
    readonly filter =
        signal<LibraryFilter>('all');

    readonly sort =
        signal<LibrarySort>('recent');

    readonly viewMode =
        signal<LibraryViewMode>('grid');
    ...
    readonly filteredStories = computed(() => {
        ...
        const filtered =
            this.storiesWithFavorites().filter(
                (story) => {
                    const matchesSearch =
    ...
    readonly statistics = computed<LibraryStatistics>(
        () => {
            ...
            return {
                ...
                reading: stories.filter(
                    (story) => story.isReading,
                ).length,

                favorites: stories.filter(
    ...
    readonly resultCount = computed(
        () => this.filteredStories().length,
    );

    load(): void {
        const view =
            this.repository.getLibrary();
