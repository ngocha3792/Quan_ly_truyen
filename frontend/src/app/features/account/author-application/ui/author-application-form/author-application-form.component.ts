...
export class AuthorApplicationFormComponent implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (!changes['draft'] || !this.draft) return;
  ...
  protected handleSubmit(): void {
    ...
    if (!this.selectedFile) this.fileError = 'Vui lòng đính kèm mẫu chương truyện.';
    if (this.form.invalid || !this.selectedFile) return;
    this.submitApplication.emit({ ...this.buildDraft(), sampleFile: this.selectedFile });
  }
