...
} from '../domain/author-application.repository';

import {
  AuthorApplicationUploadService,
} from './author-application-upload.service';
...
@Injectable()
export class AuthorApplicationApiRepository
  implements AuthorApplicationRepository
{
  ...
  private readonly baseUrl =
    `${this.config.apiBaseUrl}/author-applications`;

  private confirmedSampleRetryCache:
  ...
  getConfig(): Observable<AuthorApplicationConfig> {
    return this.http
      .get<
        ApiSuccessEnvelope<AuthorApplicationConfig>
      >(
        `${this.baseUrl}/config`,
      )
      .pipe(
  ...
  getMine(): Observable<AuthorApplicationRecord | null> {
    return this.http
      .get<
        ApiSuccessEnvelope<AuthorApplicationRecord | null>
      >(
        `${this.baseUrl}/me`,
      )
      .pipe(
  ...
  saveDraft(
    ...
    return this.http
      .put<
        ApiSuccessEnvelope<AuthorApplicationRecord>
      >(
  ...
  submit(
    ...
    const draft:
      ...
      firstWorkSynopsis:
        payload.firstWorkSynopsis,

      acceptedTerms:
        payload.acceptedTerms,

      sampleFileName:
        payload.sampleFile.name,
    };
  ...
  private resolveConfirmedSample(
    ...
    const cached =
      this.confirmedSampleRetryCache;
