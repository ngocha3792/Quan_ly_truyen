...
export type ReadingHistorySyncState =
    | 'idle'
    | 'success';
...
export class ReadingHistoryStore {
    private readonly repository =
        inject(ReadingHistoryRepository);

    private readonly viewState =
        signal<ReadingHistoryView | null>(null);

    readonly view = this.viewState.asReadonly();

    readonly query = signal('');
    readonly period =
        signal<ReadingHistoryPeriod>('all');

    readonly sort =
        signal<ReadingHistorySort>('recent');

    readonly bookmarkedIds =
        signal<readonly string[]>([]);

    readonly syncState =
        signal<ReadingHistorySyncState>('idle');
    ...
    readonly filteredHistory = computed(() => {
        ...
        const filtered = view.history.filter(
            (item) => {
                const matchesQuery =
                ...
                const matchesPeriod =
                    this.matchesSelectedPeriod(
                        item.lastReadMinutes,
                    );
        ...
        return [...filtered].sort(
            (first, second) => {
                switch (this.sort()) {
                    ...
                    default:
                        return (
                            first.lastReadMinutes -
                            second.lastReadMinutes
                        );
                }
            },
        );
