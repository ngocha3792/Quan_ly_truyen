...
import { AccountUiPreferences } from './account-profile.models';

const STORAGE_KEY =
    'truyenhub.account.ui-preferences';
...
export class AccountPreferencesStore {
    ...
    update(
        changes: Partial<AccountUiPreferences>,
    ): void {
        const nextValue = {
    ...
    private read(): AccountUiPreferences {
        ...
        try {
            const parsed = JSON.parse(
                storedValue,
            ) as Partial<AccountUiPreferences>;
