...
export class AccountStore {
    ...
    private readonly sessionsState =
        signal<AccountSessionsResponse>(
            EMPTY_SESSIONS,
        );

    private readonly securityEventsState =
        signal<AccountSecurityEventsResponse>(
    ...
    readonly sessions =
        this.sessionsState.asReadonly();

    readonly securityEvents =
        this.securityEventsState.asReadonly();
    ...
    readonly activeSessionCount = computed(
        () => this.sessionsState().total,
    );

    readonly activityCount = computed(
        () => this.securityEventsState().total,
    );
    ...
    readonly securitySummary =
        computed<AccountSecuritySummary>(() => {
            ...
            if (score >= 55) {
                return {
                    ...
                    tone: 'medium',
                    description:
                        'Bạn nên bổ sung thêm các lớp bảo mật.',
                };
            }
            ...
            return {
                ...
                tone: 'low',
                description:
                    'Tài khoản cần được kiểm tra bảo mật.',
            };
        });
