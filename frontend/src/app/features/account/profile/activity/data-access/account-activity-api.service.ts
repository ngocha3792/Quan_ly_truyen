...
export class AccountActivityApiService {
  ...
  getSecurityEvents(limit = 100): Observable<AccountSecurityEventsResponse> {
  ...
  getSessions(): Observable<ActivitySessionsResponse> {
    return this.http
      .get<ApiSuccessEnvelope<ActivitySessionsResponse>>(`${this.authUrl}/sessions`)
      .pipe(map((response) => response.data));
  }
}
