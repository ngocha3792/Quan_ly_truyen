...
export class AccountProfileStore {
  ...
  readonly completion = computed<ProfileCompletion>(() => {
    const user = this.user();

    const items = [
      {
        label: 'Ảnh đại diện',
        description: user?.avatar?.url ? 'Đã cập nhật' : 'Chưa cập nhật',
        completed: Boolean(user?.avatar?.url),
      },
      {
        label: 'Email',
        description: user?.emailVerified ? 'Đã xác minh' : 'Chưa xác minh',
        completed: Boolean(user?.emailVerified),
      },
      {
        label: 'Tên hiển thị',
        description: user?.displayName?.trim() ? 'Đã hoàn thành' : 'Chưa hoàn thành',
        completed: Boolean(user?.displayName?.trim()),
      },
      {
        label: 'Tiểu sử',
        description: user?.bio?.trim() ? 'Đã hoàn thành' : 'Chưa hoàn thành',
  ...
  save(formValue: AccountProfileFormValue, avatarFile: File | null): Observable<CurrentUser> {
    const currentUser = this.user();

    if (!currentUser) {
      return throwError(() => new Error('Không tìm thấy phiên đăng nhập.'));
    }
    ...
    const avatarRequest$ = avatarFile
      ? this.avatarUpload
          .uploadAvatar(currentUser.id, avatarFile)
          .pipe(switchMap((media) => of(media.id)))
      : of(currentUser.avatar?.id ?? null);
    ...
    return avatarRequest$.pipe(
      ...
      tap((updatedUser) => {
        this.auth.replaceCurrentUser(updatedUser);

        this.successState.set('Thông tin cá nhân đã được cập nhật.');
      }),
