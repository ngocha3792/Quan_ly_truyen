...
export class AccountAvatarUploadService {
  ...
  uploadAvatar(userId: string, file: File): Observable<ConfirmedMedia> {
    this.validateFile(file);

    return this.createIntent(userId, file).pipe(
      switchMap((intent) =>
        this.uploadToCloudinary(intent, file).pipe(
          switchMap((cloudinaryResult) => this.confirmUpload(intent, cloudinaryResult)),
        ),
      ),
    );
  }

  private createIntent(userId: string, file: File): Observable<MediaUploadIntent> {
