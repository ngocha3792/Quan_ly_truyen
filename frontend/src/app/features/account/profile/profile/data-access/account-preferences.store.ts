...
import { catchError, concatMap, EMPTY, finalize, Subject, tap } from 'rxjs';
...
export class AccountPreferencesStore {
  ...
  private readonly writeQueue = new Subject<AccountUiPreferences>();
  ...
  constructor() {
    this.writeQueue
      .pipe(
        concatMap((value) =>
          this.api.updatePreferences(value).pipe(
            tap((serverValue) => {
