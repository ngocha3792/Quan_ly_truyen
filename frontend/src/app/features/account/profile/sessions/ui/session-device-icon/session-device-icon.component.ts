...
@Component({
  selector: 'app-session-device-icon',
  ...
  template: `
    <span class="device-icon" [attr.data-browser]="browser()" [attr.aria-label]="label()">
      {{ abbreviation() }}
    </span>
  `,
  ...
  styles: `
    ...
    .device-icon {
      ...
      font-weight: 850;
      line-height: 1;
      background: linear-gradient(145deg, #46516a, #252d40);
      box-shadow: 0 8px 20px rgba(0, 0, 0, 0.2);
    }

    .device-icon[data-browser='chrome'] {
      background: conic-gradient(#ef4444 0 33%, #eab308 33% 66%, #22c55e 66% 100%);
    }
    ...
    .device-icon[data-browser='opera'] {
      background: linear-gradient(145deg, #f43f5e, #9f1239);
    }
  `,
})
export class SessionDeviceIconComponent {
  readonly browser = input<SessionBrowser>('browser');

  readonly label = input('Trình duyệt');

  readonly abbreviation = computed(() => {
    switch (this.browser()) {
      case 'chrome':
        return 'C';
