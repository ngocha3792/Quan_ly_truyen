...
export class AccountSessionsStore {
  ...
  private readonly filterState = signal<SessionFilter>('all');

  private readonly revokingIdsState = signal<ReadonlySet<string>>(new Set());
  ...
  readonly revokingIds = this.revokingIdsState.asReadonly();
  ...
  readonly currentSession = computed<AccountSessionViewModel | null>(
    () => this.sessions().find((session) => session.isCurrent) ?? null,
  );
  ...
  readonly filteredSessions = computed(() => {
    ...
    return this.otherSessions().filter((session) => {
      const matchesFilter =
        filter === 'all' ||
        (filter === 'active' && session.status === 'active') ||
        (filter === 'expired' && (session.status === 'expired' || session.status === 'revoked')) ||
        (filter === 'trusted' && session.trusted);
      ...
      const searchContent = normalizeSearchText(
        [session.title, session.subtitle, session.location, session.ipAddress ?? ''].join(' '),
      );
  ...
  readonly activeSessionCount = computed(
    () =>
      this.sessions().filter(
        (session) => session.status === 'active' || session.status === 'current',
      ).length,
  );

  readonly trustedDeviceCount = computed(
    () => this.sessions().filter((session) => session.trusted).length,
  );

  readonly latestLoginAt = computed<string | null>(() => {
    const dates = this.sessions()
      .map((session) => new Date(session.lastUsedAt).getTime())
  ...
  readonly recentLogins = computed<readonly LoginActivityViewModel[]>(() => {
    const securityActivities = this.eventsState()
      .filter((event) => {
        const action = event.action.toLowerCase();

        return action.includes('login') || action.includes('session') || action.includes('logout');
      })
      .map(toLoginActivityViewModel)
      .slice(0, 5);
  ...
  readonly revocableSessionCount = computed(
  ...
  load(force = false): void {
    ...
    forkJoin({
      ...
      securityEvents: this.api.getRecentSecurityEvents(12).pipe(
        catchError(() =>
          of({
            events: [],
            total: 0,
      ...
      .subscribe({
        next: ({ sessions, securityEvents }) => {
          this.rawSessionsState.set(sessions.sessions);

          this.eventsState.set(securityEvents.events);
  ...
  revokeSession(sessionId: string): Observable<void> {
    ...
    if (!session || !session.canRevoke) {
      return throwError(() => new Error('Phiên đăng nhập này không thể thu hồi.'));
    }
    ...
    return this.api.revokeSession(sessionId).pipe(
      tap(() => {
        this.removeSession(sessionId);

        this.successState.set(`Đã thu hồi phiên ${session.title}.`);
      }),
  ...
  private removeSession(sessionId: string): void {
    this.rawSessionsState.update((sessions) =>
      sessions.filter((session) => session.id !== sessionId),
    );
  }

  private addRevokingId(sessionId: string): void {
    const next = new Set(this.revokingIdsState());
  ...
  private removeRevokingId(sessionId: string): void {
    const next = new Set(this.revokingIdsState());
...
function mapToVoid() {
  return (source: Observable<unknown>): Observable<void> =>
