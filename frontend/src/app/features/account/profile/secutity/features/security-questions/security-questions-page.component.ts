...
@Component({
    ...
    templateUrl:
        './security-questions-page.component.html',

    styleUrl:
        './security-questions-page.component.scss',

    changeDetection:
        ChangeDetectionStrategy.OnPush,
})
export class SecurityQuestionsPageComponent
    implements OnInit {
    protected readonly store =
        inject(SecurityQuestionsStore);
        ...
        new FormGroup(
            {
                ...
                answers:
                    new FormArray<QuestionAnswerForm>(
                        [
            ...
            {
                validators:
                    distinctQuestionsValidator(),
            },
        );

    protected readonly removeForm =
        new FormGroup({
            currentPassword:
                new FormControl('', {
                    nonNullable: true,
    ...
    protected get answers():
        FormArray<QuestionAnswerForm> {
...
function distinctQuestionsValidator():
    ValidatorFn {
    return (
        ...
        if (
            !(answers instanceof FormArray)
        ) {
            return null;
        }

        const ids = answers.controls
            .map(
                (item) =>
                    item.get('questionId')
                        ?.value,
