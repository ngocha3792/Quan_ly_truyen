...
export class SecurityQuestionsStore {
  ...
  load(force = false): void {
    ...
    forkJoin({
      options: this.api.getSecurityQuestionCatalog(),
  ...
  save(request: UpdateSecurityQuestionsRequest): Observable<SecurityQuestionsState> {
    return this.execute(
      this.api.updateSecurityQuestions(request),
      'Câu hỏi bảo mật đã được cập nhật.',
    );
  }

  remove(request: RemoveSecurityQuestionsRequest): Observable<SecurityQuestionsState> {
    return this.execute(this.api.removeSecurityQuestions(request), 'Câu hỏi bảo mật đã được xóa.');
  }
