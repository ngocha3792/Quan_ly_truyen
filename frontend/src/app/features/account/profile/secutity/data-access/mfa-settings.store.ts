...
export class MfaSettingsStore {
  ...
  private readonly enrollmentState = signal<MfaEnrollment | null>(null);

  private readonly recoveryCodesState = signal<readonly string[]>([]);
  ...
  beginEnrollment(request: BeginMfaEnrollmentRequest): Observable<MfaEnrollment> {
    this.beginSubmission();

    return this.api.beginMfaEnrollment(request).pipe(
      tap((enrollment) => {
  ...
  confirmEnrollment(
    ...
    return this.api.confirmMfaEnrollment(request).pipe(
      tap((response) => {
        this.statusState.set(response.status);

        this.enrollmentState.set(null);

        this.recoveryCodesState.set(response.recoveryCodes);

        this.successState.set('Xác thực hai lớp đã được bật.');
      }),
  ...
  disable(request: VerifySensitiveActionRequest): Observable<MfaStatus> {
    this.beginSubmission();
    ...
    return this.api.disableMfa(request).pipe(
      tap((status) => {
        ...
        this.successState.set('Xác thực hai lớp đã được tắt.');
      }),
  ...
  regenerateRecoveryCodes(
    ...
    return this.api.regenerateRecoveryCodes(request).pipe(
      tap((response) => {
        this.recoveryCodesState.set(response.recoveryCodes);
        ...
        this.successState.set('Bộ mã khôi phục mới đã được tạo.');
      }),
