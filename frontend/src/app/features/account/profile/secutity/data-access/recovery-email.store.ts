...
export class RecoveryEmailStore {
  ...
  request(request: RequestRecoveryEmailRequest): Observable<RecoveryEmailStatus> {
    return this.execute(
  ...
  verify(request: VerifyRecoveryEmailRequest): Observable<RecoveryEmailStatus> {
    return this.execute(this.api.verifyRecoveryEmail(request), 'Email khôi phục đã được xác minh.');
  }

  resend(): Observable<RecoveryEmailStatus> {
    return this.execute(this.api.resendRecoveryEmailCode(), 'Mã xác minh mới đã được gửi.');
  }

  remove(request: RemoveRecoveryEmailRequest): Observable<RecoveryEmailStatus> {
    return this.execute(this.api.removeRecoveryEmail(request), 'Email khôi phục đã được xóa.');
  }
