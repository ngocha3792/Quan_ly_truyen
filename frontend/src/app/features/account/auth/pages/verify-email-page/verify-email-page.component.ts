...
export class VerifyEmailPageComponent implements OnInit {
  ...
  protected readonly message = signal('Đang xác minh địa chỉ email của bạn...');

  protected readonly verifiedAt = signal<string | null>(null);
  ...
  ngOnInit(): void {
    ...
    if (!token) {
      ...
      this.message.set('Link xác minh không có token hoặc không hợp lệ.');
    ...
    this.authApi.verifyEmail(token).subscribe({
      next: (result) => {
        ...
        this.verifiedAt.set(new Date(result.verifiedAt).toLocaleString('vi-VN'));
      },
