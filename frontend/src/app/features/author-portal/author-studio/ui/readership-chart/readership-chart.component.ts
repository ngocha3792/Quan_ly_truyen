      ...
      <header>
        <div>
          <h2>
            ...
            <span title="Lượt xem trong khoảng thời gian đã chọn">
              i
            </span>
          </h2>
...
export class ReadershipChartComponent {
    private readonly pointsState =
        signal<readonly ReadershipChartPoint[]>(
            [],
        );
    ...
    readonly periodChange =
        new EventEmitter<AuthorStudioPeriod>();
    ...
    protected readonly positionedPoints =
        computed(() => {
            const points =
                this.pointsState();
    ...
    protected get selectedPeriodLabel():
        string {
        switch (this.period) {
            case '7d':
                return '7 ngày qua';
    ...
    protected handlePeriodChange(
        ...
        this.periodChange.emit(
            select.value as AuthorStudioPeriod,
        );
    }
}