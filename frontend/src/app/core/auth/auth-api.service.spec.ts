...
describe('AuthApiService', () => {
  ...
  it('register gửi idempotency key và skip refresh', async () => {
    const promise = firstValueFrom(
      service.register({
        email: 'new@truyenhub.test',
  ...
  it('refresh luôn skip auth refresh interceptor', async () => {
  ...
  it('resend verification dùng đúng contract', async () => {
    const promise = firstValueFrom(service.resendVerification('USER@EXAMPLE.COM'));

    const request = http.expectOne('/api/v1/auth/resend-verification');
    ...
    request.flush(
      successEnvelope({
        ...
        message: 'Nếu email hợp lệ, hệ thống sẽ gửi email xác minh.',
      }),
    );
  ...
  it('confirm email change dùng token và skip refresh', async () => {
    const promise = firstValueFrom(service.confirmEmailChange('email-change-token'));

    const request = http.expectOne('/api/v1/auth/change-email/confirm');
