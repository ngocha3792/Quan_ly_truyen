...
describe(
    ...
    () => {
        ...
        it(
            'anonymous user được redirect tới login kèm returnUrl',
            async () => {
        ...
        it(
            'authenticated user được đi qua authenticatedGuard',
            async () => {
        ...
        it(
            'guard chờ AuthStore bootstrap khi status đang idle',
            async () => {
        ...
        it(
            'USER không được vào route AUTHOR',
            async () => {
                authenticate(
                    createCurrentUser({
                        roles: ['USER'],
                ...
                expect(serialize(result)).toBe(
                    '/khong-co-quyen?reason=role&from=%2Fauthor-studio',
                );
            },
        );

        it(
            'AUTHOR được vượt qua roleGuard',
            async () => {
                authenticate(
                    createCurrentUser({
                        roles: ['AUTHOR'],
        ...
        it(
            'role guard xử lý role không phân biệt hoa thường',
            async () => {
                authenticate(
                    createCurrentUser({
                        roles: ['author'],
        ...
        it(
            'permissionGuard cho phép user có permission',
            async () => {
                authenticate(
                    createCurrentUser({
        ...
        it(
            'permissionGuard từ chối user thiếu permission',
            async () => {
                authenticate(
                    createCurrentUser({
                        permissions: [],
                ...
                expect(serialize(result)).toBe(
                    '/khong-co-quyen?reason=permission&from=%2Fthu-vien',
                );
            },
        );

        it(
            'permissionGuard không phân biệt hoa thường',
            async () => {
                authenticate(
                    createCurrentUser({
                        permissions: ['LIBRARY.MANAGE.OWN'],
        ...
        async function executeGuard(
            ...
            if (isObservable(result)) {
                return firstValueFrom(result) as Promise<
                    boolean | UrlTree
                >;
            }
