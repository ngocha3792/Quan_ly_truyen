...
describe('AuthStore', () => {
  ...
  it('không gọi refresh khi browser không có session hint', () => {
    sessionHint.shouldAttemptRefresh.mockReturnValue(false);
  ...
  it('restore session bằng refresh + me', () => {
    ...
    expect(tokens.accessToken()).toBe('access-token-v2');

    expect(sessionHint.markSessionPresent).toHaveBeenCalled();
  });

  it('bootstrap chỉ xóa session hint khi backend từ chối phiên', () => {
    sessionHint.shouldAttemptRefresh.mockReturnValue(true);
    ...
    expect(sessionHint.markSessionAbsent).toHaveBeenCalledTimes(1);
  });

  it('bootstrap giữ session hint và cho retry khi lỗi tạm thời', () => {
    const user = createCurrentUser();
    ...
    refreshService.refreshAccessToken
      ...
      .mockImplementationOnce(() => {
        tokens.set('access-token-after-retry');

        return of('access-token-after-retry');
      });
  ...
  it('bootstrap chỉ được chạy một lần', () => {
    ...
    expect(sessionHint.shouldAttemptRefresh).toHaveBeenCalledTimes(1);
  });

  it('login lưu access token rồi gọi /me', async () => {
    const user = createCurrentUser();

    api.login.mockReturnValue(of(createLoginResponse('access-token-login', user)));
    ...
    expect(tokens.accessToken()).toBe('access-token-login');
  ...
  it('login fail phải clear local session', async () => {
    tokens.set('old-token');

    api.login.mockReturnValue(throwError(() => new Error('Sai mật khẩu')));
  ...
  it('refresh session thành công phải cập nhật CurrentUser', async () => {
    const user = createCurrentUser({
      displayName: 'User After Refresh',
    });
    ...
    const result = await firstValueFrom(store.refreshSession());

    expect(result).toEqual(user);

    expect(store.user()?.displayName).toBe('User After Refresh');
  ...
  it('MFA enrollment confirm phải hoàn tất login và giữ recovery codes', async () => {
    const user = createCurrentUser();

    api.confirmMfaEnrollment.mockReturnValue(
      of({
        ...createLoginResponse('mfa-access', user),
    ...
    expect(result.recoveryCodes).toEqual(['RECOVERY-1', 'RECOVERY-2']);

    expect(store.isAuthenticated()).toBe(true);

    expect(tokens.accessToken()).toBe('mfa-access');
  });
});
