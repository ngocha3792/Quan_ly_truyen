...
function dates(fromKey: string, toKey: string): string[] {
  ...
  for (
    let day = new Date(`${fromKey}T00:00:00.000Z`);
    day <= end;
...
async function main(): Promise<void> {
  ...
  for (const dateKey of dates(from, to)) {
    const storyRows = await prisma.$queryRaw<
      Array<{
        storyId: string;
        views: bigint;
    ...
    for (const row of storyRows) {
      const current = await prisma.storyDailyStat.findUnique({
        where: { storyId_date: { storyId: row.storyId, date } },
      });
      const mismatch =
        !current ||
        current.viewCount !== row.views ||
        current.uniqueReaders !== Number(row.uniqueReaders) ||
        current.readingStartCount !== Number(row.starts) ||
        current.completionCount !== Number(row.completions) ||
        current.readingSeconds !== row.readingSeconds;
      ...
      console.log(
        ...
        dateKey,
        row.storyId,
        `views ${current?.viewCount ?? 'missing'} -> ${row.views}`,
        `unique ${current?.uniqueReaders ?? 'missing'} -> ${row.uniqueReaders}`,
      );
      if (apply)
        await prisma.storyDailyStat.upsert({
          where: { storyId_date: { storyId: row.storyId, date } },
          create: {
            storyId: row.storyId,
            date,
    ...
    for (const row of chapterRows) {
      const current = await prisma.chapterDailyStat.findUnique({
      ...
      if (apply)
        await prisma.chapterDailyStat.upsert({
          where: { chapterId_date: { chapterId: row.chapterId, date } },
          create: {
  ...
  console.log(
    `${apply ? 'Applied' : 'Dry run'}: ${mismatches} mismatch(es) in ${from}..${to}.`,
  );
}
