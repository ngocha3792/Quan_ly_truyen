...
async function main(): Promise<void> {
  ...
  for (const session of sessions) {
    ...
    const story = byStory.get(storyKey) ?? {
    ...
    story.starts += 1;
    story.completions += session.completed ? 1 : 0;
    story.seconds += duration;
    story.readers.add(session.userId);
    byStory.set(storyKey, story);
    const chapter = byChapter.get(chapterKey) ?? {
    ...
    chapter.starts += 1;
    chapter.completions += session.completed ? 1 : 0;
    chapter.seconds += duration;
    chapter.readers.add(session.userId);
    byChapter.set(chapterKey, chapter);
  }
  console.log(`Reading sessions found: ${sessions.length}`);
  console.log(
    `Story daily rows: ${byStory.size}; chapter daily rows: ${byChapter.size}; timezone: ${timeZone}`,
  );
  if (!apply) {
    console.log(
      'Dry run only. Use --apply to SET canonical session-derived fields.',
    );
    return;
  }
    ...
    for (const row of byStory.values()) {
      await tx.storyDailyStat.upsert({
        ...
        create: {
          storyId: row.storyId,
          date: row.date,
          uniqueReaders: row.readers.size,
          readingStartCount: row.starts,
          completionCount: row.completions,
          readingSeconds: row.seconds,
        },
        update: {
          uniqueReaders: row.readers.size,
          readingStartCount: row.starts,
  ...
  console.log(
    'Backfill applied. View metrics were not invented from ReadingSession data.',
  );
}
