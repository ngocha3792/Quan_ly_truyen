...
describe(
  ...
  () => {
    ...
    it(
      'hai PATCH preferences concurrent không được làm mất thay đổi của nhau',
    ...
    it(
      'suspend user phải revoke toàn bộ active sessions trong cùng operation',
    ...
    it(
      'activate lại user không được làm session cũ sống lại',
    ...
    it(
      'assign ADMIN phải idempotent và không tạo duplicate UserRole',
      ...
      async () => {
        ...
        const first =
          await managedRepository.assignManagedUserRole({
            actorUserId:
              actor.id,
    ...
    it(
      'remove ADMIN phải revoke toàn bộ session của target',
      ...
      async () => {
        ...
        const result =
          await managedRepository.removeManagedUserRole({
            actorUserId:
              actor.id,
        ...
        expect(
          freshSession.accessTokenVersion,
        ).toBe(
          1,
        );

        expect(
          freshSession.refreshTokenVersion,
        ).toBe(
          1,
        );
      },
    ...
    async function createUser(
      ...
      const user =
        await prisma.user.create({
          data: {
            email:
              `${label}.${sequence}.${runId}@example.test`,
