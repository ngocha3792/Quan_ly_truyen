...
describe('Phase 6 reader analytics', () => {
  ...
  it('ingests idempotently, hashes anonymous identity and rejects spoofed chapter context', async () => {
    ...
    const analyticsConfig = {
      ...actualConfig.getOrThrow<Record<string, unknown>>('analytics'),
      enabled: true,
    };
    const fakeConfig = {
    ...
    const raw = await prisma.readerAnalyticsEvent.findUniqueOrThrow({
      where: { eventId },
    });
    expect(raw.viewerKeyHash).toBe(
      localIdentity.hashAnonymous(anonymousReaderId),
    );
    ...
    await expect(
      ingestion.ingest({
        anonymousReaderId,
        events: [{ ...event, eventId: randomUUID(), storyId: other.storyId }],
      }),
  ...
  it('aggregates accepted events once even when the same worker batch is replayed', async () => {
    const { authorId, storyId, chapterId } =
      await createPublishedStory('replay');
    const viewer = identity.hashAnonymous(randomUUID());
    const sessionId = randomUUID();
    const occurredAt = new Date();
    const events = await Promise.all([
      createEvent({
        type: ReaderAnalyticsEventType.STORY_VIEW,
        viewer,
      ...
      createEvent({
        type: ReaderAnalyticsEventType.READING_STARTED,
        viewer,
        sessionId,
        storyId,
        chapterId,
      ...
      createEvent({
        type: ReaderAnalyticsEventType.READING_COMPLETED,
        viewer,
        sessionId,
        storyId,
        chapterId,
    ...
    const stats = await prisma.storyDailyStat.findFirstOrThrow({
      where: { storyId },
    });
    ...
    expect(
      await prisma.readerAnalyticsEvent.count({
  ...
  it('recomputes daily unique viewers canonically rather than incrementing repeats', async () => {
    ...
    await reconcile.recomputeUniqueReaders(dateKey);
    expect(
      (await prisma.storyDailyStat.findFirstOrThrow({ where: { storyId } }))
        .uniqueReaders,
    ).toBe(2);
    expect(
      (await prisma.chapterDailyStat.findFirstOrThrow({ where: { chapterId } }))
        .uniqueReaders,
    ).toBe(2);
  });
  ...
  it('uses the configured timezone for daily bucket boundaries', () => {
    expect(
      analyticsDateKey(
        new Date('2026-08-16T16:59:00.000Z'),
        'Asia/Ho_Chi_Minh',
      ),
    ).toBe('2026-08-16');
    expect(
      analyticsDateKey(
        new Date('2026-08-16T17:01:00.000Z'),
        'Asia/Ho_Chi_Minh',
      ),
    ).toBe('2026-08-17');
  });
  ...
  it('retention cleanup deletes only old processed raw events', async () => {
    ...
    const processed = await prisma.readerAnalyticsEvent.create({
      data: {
        eventId: randomUUID(),
        type: ReaderAnalyticsEventType.STORY_VIEW,
        viewerKeyHash: identity.hashAnonymous(randomUUID()),
        sessionId: randomUUID(),
        storyId,
        occurredAt: old,
        receivedAt: old,
        processedAt: new Date(old.getTime() + 1000),
      },
    });
    const unprocessed = await prisma.readerAnalyticsEvent.create({
      data: {
        eventId: randomUUID(),
        type: ReaderAnalyticsEventType.STORY_VIEW,
        viewerKeyHash: identity.hashAnonymous(randomUUID()),
    ...
    expect(
      await prisma.readerAnalyticsEvent.findUnique({
  ...
  it('enforces author ownership and returns null rate without a reading-start denominator', async () => {
    ...
    const ownView = await createEvent({
      type: ReaderAnalyticsEventType.STORY_VIEW,
      viewer: identity.hashAnonymous(randomUUID()),
      sessionId: randomUUID(),
      storyId: own.storyId,
      occurredAt: new Date(),
    });
  ...
  async function createEvent(input: {
    ...
    return prisma.readerAnalyticsEvent.create({
      data: {
        eventId: randomUUID(),
        type: input.type,
        viewerKeyHash: input.viewer,
  ...
  async function createPublishedStory(prefix: string) {
    ...
    const story = await prisma.story.create({
      data: {
        authorId: user.id,
        title: marker,
        slug: unique('story').toLowerCase(),
        synopsis: 'analytics test',
  ...
  async function cleanup(): Promise<void> {
    ...
    await prisma.readerAnalyticsEvent.deleteMany({
