...
export interface InfrastructureDiagnostics {
  database: { status: DiagnosticStatus };
  redis: { status: DiagnosticStatus };
  queue: { status: DiagnosticStatus };
  mail: { status: DiagnosticStatus };
  cloudinary: { status: DiagnosticStatus };
  outbox: {
...
export class InfrastructureDiagnosticsService {
  ...
  private async redisStatus(
    ...
    if (!this.redisClient) return 'down';
    try {
      ...
      await this.prisma.$transaction([
        ...
        this.prisma.outboxEvent.count({
          where: {
            ...
            processedAt: { gte: failedSince },
          },
        }),
      ]);
    return {
      status: staleProcessing > 0 ? 'down' : 'up',
      pendingTooOld,
      staleProcessing,
      failedRecently,
    };
