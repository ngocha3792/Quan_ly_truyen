...
import type { CsrfTokenPort } from '../../../application/ports';

import { AuthCookieManager } from './auth-cookie.manager';

describe('AuthCookieManager OAuth state cookie', () => {
  const authConfig = {
    refreshCookie: {
      name: 'refresh_token',
      secure: true,
  ...
  const service = new AuthCookieManager(
    new ConfigService({ auth: authConfig }),
    { issue: jest.fn() } as unknown as CsrfTokenPort,
  );
