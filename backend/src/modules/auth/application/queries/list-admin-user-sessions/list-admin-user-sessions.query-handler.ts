...
export class ListAdminUserSessionsQueryHandler {
  ...
  async execute(query: ListAdminUserSessionsQuery): Promise<readonly AdminSessionView[]> {
    if (!(await this.persistence.userExists(query.userId))) throw userNotFound(query.userId);
    return this.persistence.listSessions(query.userId);
  }
}
function userNotFound(id: string) {
  return new ResourceNotFoundException({ code: 'ADMIN_USER_SECURITY_USER_NOT_FOUND', resource: 'người dùng', identifier: id, message: 'Không tìm thấy người dùng cần quản lý bảo mật' });
}
