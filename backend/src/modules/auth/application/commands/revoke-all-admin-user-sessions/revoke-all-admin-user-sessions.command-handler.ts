...
export class RevokeAllAdminUserSessionsCommandHandler {
  ...
  async execute(command: RevokeAllAdminUserSessionsCommand): Promise<number> {
    if (!(await this.persistence.userExists(command.userId))) throw new ResourceNotFoundException({ code: 'ADMIN_USER_SECURITY_USER_NOT_FOUND', resource: 'người dùng', identifier: command.userId, message: 'Không tìm thấy người dùng cần quản lý bảo mật' });
    return this.persistence.revokeAllSessions({ ...command, revokedAt: new Date() });
  }
}
