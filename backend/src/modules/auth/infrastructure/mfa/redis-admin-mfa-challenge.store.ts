...
import type {
  ...
  CreateAdminMfaChallengeInput,
} from '../../application/ports';
import { InvalidMfaTicketException } from '../../domain/exceptions';

export interface StoredAdminMfaChallenge extends CreateAdminMfaChallengeInput {
...
@Injectable()
export class RedisAdminMfaChallengeStore implements AdminMfaChallengePort {
  ...
  async create(
    ...
    return { ticket, mode: input.mode, expiresAt };
  }

  async read(ticket: string): Promise<StoredAdminMfaChallenge> {
  ...
  async savePendingSecret(
    ...
    const result = await this.requireRedis().eval(
      ...
      if value.mode ~= 'enroll' then return nil end
      if value.pendingSecret then return value.pendingSecret end
      value.pendingSecret = ARGV[1]
  ...
  async consume(ticket: string): Promise<StoredAdminMfaChallenge> {
  ...
  async recordFailure(ticket: string): Promise<number> {
    const result = await this.requireRedis().eval(
      ...
      if not raw then return nil end
      local value = cjson.decode(raw)
      value.attemptsRemaining = tonumber(value.attemptsRemaining) - 1
  ...
  private key(ticket: string): string {
    return `auth:mfa:preauth:${sha256(ticket)}`;
  }
...
function parseChallenge(raw: string): StoredAdminMfaChallenge {
  try {
    const value = JSON.parse(raw) as StoredAdminMfaChallenge;
    if (!value.userId || !value.mode || !value.expiresAt) {
