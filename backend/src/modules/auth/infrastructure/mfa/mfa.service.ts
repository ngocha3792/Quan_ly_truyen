...
export interface MfaSettingsEnrollmentResult {
  enrollmentId: string;
...
export interface MfaSettingsConfirmationResult {
  status: MfaStatusRecord;
...
export class MfaService {
  ...
  async beginPreAuthEnrollment(ticket: string): Promise<MfaEnrollmentResult> {
  ...
  async getStatus(userId: string | undefined): Promise<MfaStatusRecord> {
  ...
  ): Promise<MfaSettingsEnrollmentResult> {
    const normalizedUserId = this.requireUserId(userId);
  ...
  ): Promise<MfaSettingsConfirmationResult> {
    const normalizedUserId = this.requireUserId(userId);
