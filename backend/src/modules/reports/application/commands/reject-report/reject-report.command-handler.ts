...
export class RejectReportCommandHandler {
  ...
  execute(command: RejectReportCommand) {
    ...
    return this.repository.close({ actorId: command.actorId, reportId: command.reportId, note, status: 'REJECTED', auditAction: 'comment.report.rejected', audit: command.audit });
  }
}
