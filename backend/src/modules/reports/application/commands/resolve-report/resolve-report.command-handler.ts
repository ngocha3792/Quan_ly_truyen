...
export class ResolveReportCommandHandler {
  ...
  execute(command: ResolveReportCommand) {
    ...
    return this.repository.close({ actorId: command.actorId, reportId: command.reportId, note, status: 'RESOLVED', auditAction: 'comment.report.resolved', audit: command.audit });
  }
}
