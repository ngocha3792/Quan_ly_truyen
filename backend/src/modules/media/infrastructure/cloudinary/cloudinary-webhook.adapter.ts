...
@Injectable()
export class CloudinaryWebhookAdapter implements MediaWebhookPort {
  private readonly logger = new Logger(CloudinaryWebhookAdapter.name);
  constructor(
  ...
  async handle(input: MediaWebhookInput): Promise<MediaWebhookResult> {
    if (
      !this.cloudinary ||
