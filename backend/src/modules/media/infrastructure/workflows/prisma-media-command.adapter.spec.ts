...
import { PrismaMediaCommandAdapter } from './prisma-media-command.adapter';
import { MediaPublicIdPolicy } from '../../domain/policies/media-public-id.policy';

describe('PrismaMediaCommandAdapter upload intent', () => {
  const principal = {
  ...
  const service = new PrismaMediaCommandAdapter(
    prisma as never,
    storage as never,
    new ConfigService({
      cloudinary: { rootFolder: 'root', uploadIntentTtlSeconds: 300 },
    }),
    new MediaPublicIdPolicy(),
    ownership as never,
  );
