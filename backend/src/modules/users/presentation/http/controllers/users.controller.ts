...
export class UsersController {
  constructor(
    private readonly getCurrentUserProfile: GetCurrentUserProfileQueryHandler,

    private readonly updateCurrentUserProfile: UpdateCurrentUserProfileCommandHandler,

    private readonly getCurrentUserPreferences: GetCurrentUserPreferencesQueryHandler,

    private readonly updateCurrentUserPreferences: UpdateCurrentUserPreferencesCommandHandler,
  ) {}

  @Get('me')
  @RequirePermissions(PermissionCode.USER_PROFILE_READ)
  ...
  @Patch('me')
  @RequirePermissions(PermissionCode.USER_PROFILE_UPDATE)
  @Header('Cache-Control', 'private, no-store')
  async updateMe(
    @CurrentUserId()
    userId: string | undefined,
    ...
    const result = await this.updateCurrentUserProfile.execute(
  ...
  async getPreferences(
    ...
    const result = await this.getCurrentUserPreferences.execute(
      new GetCurrentUserPreferencesQuery(userId),
    );

    return toUserPreferencesResponse(result);
  }
  ...
  async updatePreferences(
    ...
    const result = await this.updateCurrentUserPreferences.execute(
