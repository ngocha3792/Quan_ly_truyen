  ...
  async execute(
    ...
    const displayName =
      command.displayName === undefined
        ? undefined
        : UserDisplayNameValueObject.create(command.displayName).value;

    const bio =
      command.bio === undefined
        ? undefined
        : UserBioValueObject.create(command.bio).value;
    ...
    if (
      ...
      const current = await this.reader.findProfileByUserId(userId);

      if (!current) {
        throw new UserProfileUnavailableException();
      }

      return UserProfileResultMapper.toDto(current);
    }

    const result = await this.persistence.updateProfile({
      userId,
    ...
    switch (result.status) {
      case 'updated':
        return UserProfileResultMapper.toDto(result.profile);
