...
import { GetCurrentUserPreferencesQuery } from './get-current-user-preferences.query';
...
export class GetCurrentUserPreferencesQueryHandler {
  ...
  async execute(
    ...
    const preferences = await this.reader.findPreferencesByUserId(query.userId);

    if (!preferences) {
      throw new UserProfileUnavailableException();
    }

    return UserPreferencesResultMapper.toDto(preferences);
  }
}
