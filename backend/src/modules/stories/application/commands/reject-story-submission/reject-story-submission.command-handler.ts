...
@Injectable()
export class RejectStorySubmissionCommandHandler {
  constructor(
    @Inject(STORY_PERSISTENCE_PORT)
    private readonly persistence: StoryPersistencePort,
  ) {}

  async execute(command: RejectStorySubmissionCommand) {
    const reviewerId = requireReviewerId(command.reviewerId);
    const result = await this.persistence.rejectSubmission({
      reviewerId,
      submissionId: command.submissionId,
      reviewerNote: command.reviewerNote.trim(),
      reviewedAt: new Date(),
      audit: {
