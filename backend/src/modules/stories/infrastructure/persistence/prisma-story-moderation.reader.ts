...
import type {
  ListStorySubmissionsInput,
  ListStorySubmissionsResult,
  StoryModerationReaderPort,
...
export class PrismaStoryModerationReader implements StoryModerationReaderPort {
  ...
  async listStorySubmissions(
    ...
    try {
      ...
      const direction: Prisma.SortOrder = input.sort.endsWith(':asc')
        ? 'asc'
        : 'desc';
      ...
      return {
        ...
        items: records.map((record) => ({
          ...
          author: {
            id: record.story.author.userId,
            displayName: record.story.author.user.displayName,
            slug: record.story.author.slug,
          },
          ...
          rejectionReason:
            record.status === SubmissionStatus.REJECTED
              ? record.reviewerNote
              : null,
          chapterCount: record.story.chapterCount,
        })),
      };
    } catch (error: unknown) {
      throw mapPrismaError(error, {
        operation: 'admin-story-submission-list',
        resource: 'Yêu cầu duyệt truyện',
      });
    }
  }
  async getStorySubmission(
    submissionId: string,
  ): Promise<StorySubmissionDetailRecord | null> {
    try {
      const record = await this.prisma.storySubmission.findUnique({
        ...
        select: {
          ...
          story: {
            select: {
              ...
              status: true,
              visibility: true,
              coverMedia: { select: { secureUrl: true, publicUrl: true } },
              author: {
                select: {
                  userId: true,
                  penName: true,
      ...
      const audit = await this.prisma.auditLog.findMany({
        where: { entityType: 'story', entityId: record.story.id },
      ...
      return {
        ...
        author: {
          id: record.story.author.userId,
          displayName: record.story.author.user.displayName,
          penName: record.story.author.penName,
          slug: record.story.author.slug,
          status: record.story.author.lifecycleStatus,
        },
        story: {
          id: record.story.id,
          title: record.story.title,
          slug: record.story.slug,
          synopsis: record.story.synopsis,
          status: record.story.status,
          visibility: record.story.visibility,
          coverUrl:
            record.story.coverMedia?.secureUrl ??
            record.story.coverMedia?.publicUrl ??
            null,
          categories: record.story.categories.map((item) => item.category),
          tags: record.story.tags.map((item) => item.tag),
        },
        chapters: record.story.chapters.map((chapter) => ({
          id: chapter.id,
        ...
        submissionHistory: record.story.submissions.map((submission) => ({
          id: submission.id,
      ...
      throw mapPrismaError(error, {
        operation: 'admin-story-submission-detail',
        resource: 'Yêu cầu duyệt truyện',
      });
    }
  }
