...
export class ListFollowingQueryHandler {
  ...
  execute(query: ListFollowingQuery) {
    ...
    const authorIds = query.input.authorIds?.length ? [...new Set(query.input.authorIds)] : undefined;
    if (authorIds && authorIds.length > 50) throw new InvalidInputException({ code: 'FOLLOW_AUTHOR_IDS_LIMIT_EXCEEDED', message: 'Chỉ được kiểm tra tối đa 50 tác giả mỗi lần' });
