...
import { ModerationReportMismatchException, normalizeModerationReason, normalizeWarningMessage } from '../../../domain';
...
export class WarnUserCommandHandler {
  ...
  async execute(command: WarnUserCommand) {
    const result = await this.persistence.warnUser({ ...command, reason: normalizeModerationReason(command.reason), message: normalizeWarningMessage(command.message) });
    throwFailure(result, command.commentId, command.reportId);
    if (result.status !== 'warned') throw new CommentNotFoundException(command.commentId);
    this.metrics.record('warn');
    return { success: true as const, commentId: result.commentId, warnedUserId: result.warnedUserId };
  }
}
